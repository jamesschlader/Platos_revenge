var plato = {
  //Here's the start of the game object.
  ...
  words: [
    ...
    "conclusion",
    "theory"
  ], //small list of words. In the future, I'd like this to be populated by a data base that also includes the definitions. That way, hints can be offered and the word can be properly explained at the end of the game.
  word: "",
  used: [],
  guesses: 0,
  ...
  buildWordArea() {
    //This method constructs the word chosen for guessing into the content area of the screen so that the player can track their progress toward victory.
    ...
    for (let i = 0; i < plato.word.length; i++) {
  ...
  affixLetter(letter) {
    //This method places the player's letter guess into the designated area of the screen.
    // Step 1: Empty the area if it is already holding a letter.
    const r = document.getElementById("letter");
    if (r) {
      r.remove(r);
    }
    // Step 2: Round up the element that the letter will go into.
    const x = document.getElementById("dropTarget");
    ...
    let z = document.createTextNode(letter.key.toUpperCase());
    y.appendChild(z);
    x.appendChild(y);
  },

  getGuess() {
    // This method is the heart of the object. It holds the eventListener, checks its value, and sends the letter to the appropriate method(s) for display.

    function check(letter) {
      //This is the callback function for the eventListener.
      //This function checks the user's input letter in three ways:
      const target = plato.word;
      const regEx = /[^a-z]/i;
      ...
      if (regEx.test(letter.key)) {
        ...
        //The antecedent condition merely finds out whether the guessed letter is in the word or not. Actual processing of how many letter matches there are is left for the showLetters method.
  ...
  showLetters(letter) {
    //This method reveals a success guess by changing the style of the h1 holding the letter from hidden to visible.
  ...
  usedLetters(letter) {
    //This method displays any unsucessful alpha guesses in the appropriate display space.
  ...
  showProgress(letter) {
    //This method displays how close to losing the user is.
    //Increment the number of failed guesses.
    plato.guesses++;
    const x = document.getElementById("progressArea");
  ...
  removeParagraph(id) {
    //This method cleans out a display area of all its created elements.
  ...
  gameOver() {
    ...
    //Eventually, I plan to allow the user to decide the level of difficulty in which case the losing condition will have a different test number.
    ...
    if (plato.guesses === 5) {
      ...
      for (let i = 0; i < y.length; i++) {
        if (y[i].style.visibility === "visible") {
          counter++;
        }
      }
  ...
  restart() {
    //This method resets the values of the global variables used throughout the game and calls the methods that drive the game.
    plato.score();
    plato.guesses = 0;
    plato.used = [];
    plato.chooseWord();
  ...
  score() {
    //This method updates the scoreboard after each game.
    //Clear out the old scores...
    plato.removeParagraph("wins");
    plato.removeParagraph("losses");
    //Get the current scores...
    const x = document.getElementById("wins");
    const y = document.getElementById("losses");
    ...
    const node1 = document.createTextNode("Wins: " + plato.wins);
    const node2 = document.createTextNode("Losses: " + plato.losses);
...
var messages = {
  //This method holds the various message that will give feedback to the user during the game. That way, I don't have to use alerts or prompts.
  rules:
    "Your challenge is to spell out a Philosophical Vocabulary Word before your entire body hangs. If you succeed, you will have proven your worth. If you fail, you hang.",
  ...
  win:
    "You solved the word before hanging. Looks like you're a friend of Socrates after all...",
  lose: "Ha! I knew it! Hang for your sins against Wisdom!",

  billboard(text) {
    //This method actually does the business of showing the messages to the screen.
    ...
    const bill = document.getElementById("billboard-main");
    bill.style.visibility = "hidden";
    const broadcast = document.getElementById("alert");
    ...
    if (text === messages.win) {
      ...
      //Otherwise, the messasge lives in the upper left quadrant of the game board.
      broadcast.appendChild(announcement);
    }
  },

  feedback() {
    //Calls the appropriate methods to start the game over again. In the future, this method will be used to allow the user to exit the game.As it is now, it is only called once at the time the page loads.
    plato.score();
    plato.guesses = 0;
    plato.chooseWord();
    plato.buildWordArea();
